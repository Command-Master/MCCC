...
Atom* parseStr(int c) {
  ...
  str_buf[n] = 0;
  ungetChar(c);
  if (n == 3 && str_buf[0] == 'n' && str_buf[1] == 'i' && str_buf[2] == 'l')
    return NULL;
  int* str = calloc(n + 1, sizeof(char));
  int i;
  for (i = 0; i < n; i++) {
    str[i] = str_buf[i];
  }
...
Atom* eval(Atom* a, Table* val) {
  ...
  if (hd->type == LAMBDA) {
    ...
    Table** nval = calloc(sizeof(Table*), 1);
    List* params = s->tail;
    while (args) {
